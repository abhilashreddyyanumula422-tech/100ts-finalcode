...
from .models import Review
from .serializers import ReviewSerializer
from .utils import make_admin_token
        ...
        if admin:
            if admin.password == password:
                return Response({
                    "message": "Admin Login successful",
                    "type": "admin",
                    # Previously: no token was issued here at all, so every
                    # "Authorization: Token ..." header the frontend sent was
                    # literally "Token undefined" — nothing was ever actually
                    # being verified server-side. make_admin_token() issues a
                    # real, signed, 7-day token (see utils.py).
...
def get_applications(request):
    ...
    for app in apps:
        ...
        data.append({
            ...
            "assigned": ", ".join([a.agent.name for a in agent_assignments if a.agent]) if agent_assignments else "Unassigned",
            "delivery": "Standard Courier",
            "district": getattr(app, 'district', 'N/A'),
            "user_acknowledged": app.user_acknowledged,
            "documentsList": [
                {"id": doc.id, "name": doc.name, "status": "Verified", "url": request.build_absolute_uri(doc.file.url)}
                for doc in app.documents.all()
