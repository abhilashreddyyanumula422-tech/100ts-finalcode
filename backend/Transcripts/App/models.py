...
class Application(models.Model):
    ...
    total_amount_edit_reason = models.TextField(null=True, blank=True)
    extra_amount = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    extra_paid_amount = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    extra_payment_status = models.CharField(max_length=20, default="NONE")
    extra_payment_reason = models.TextField(null=True, blank=True)
...
class Invoice(models.Model):
    ...
    application = models.ForeignKey('Application', on_delete=models.CASCADE, related_name='invoices')
    amount_paid = models.DecimalField(max_digits=10, decimal_places=2)
    pdf_file = models.FileField(upload_to='invoices/', null=True, blank=True)
    whatsapp_status = models.CharField(max_length=50, default='PENDING')
    email_status = models.CharField(max_length=50, default='PENDING')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"{self.invoice_number} ({self.payment.order_id})"
